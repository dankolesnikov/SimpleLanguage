...
class SL1Parser extends JavaTokenParsers {
  ...
  def block: Parser[Block] = rep(valdef | defdef) ~ expr ^^ {
    case lst ~ e => Block(lst, e)
  }

  // In cons cannot use expr because it will give infinite recursion
  def cons: Parser[Expr] = (expr2 <~ opt("::" | ident)) ~ expr ^^ {
    case expr ~ (lst: Expr) => Cons(expr, lst)
    //case expr ~ None => Cons(expr,new Expr)
  }

  def expr: Parser[Expr] = cons | (("if" ~> ("(" ~> expr <~ ")") ~ block <~ "else") ~ block) ^^ {
  ...
  // This is where function calls are parsed
  def valOrFuncall = valOrFun ~ rep( "(" ~> repsep(expr, ",") <~ ")" ) ^^ {
    case expr ~ Nil => expr
    case expr ~ args =>  (expr /: (args)) (Funcall(_, _)) // foldLeft
  }
...
object SL1 extends App {
  ...
  def evalBlock(block : Block, symbols : List[(String, Any)]) : Any =
    eval(block.expr, spy((symbols /: block.defs) { evalDef(_, _) } ))

  val initial_symbol_table =
    ("head", ListOp(l => l.head)) ::
      ("tail", ListOp(l => l.tail)) ::
      ("isEmpty", ListOp(l => if (l.isEmpty) 1 else 0)) ::
      ("Nil", List()) :: Nil
  ...
  parseResult match {
    ...
    case parser.Success(result, next) => println(evalBlock(result: Block, initial_symbol_table))
